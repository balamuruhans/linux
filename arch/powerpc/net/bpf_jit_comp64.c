...
static void bpf_jit_emit_tail_call(u32 *image, struct codegen_context *ctx, u32 out)
{
	 ...
	 *   goto out;
	 */
	EMIT(PPC_LWZ(b2p[TMP_REG_1], b2p_bpf_array, offsetof(struct bpf_array, map.max_entries)));
	...
	EMIT(PPC_ADD(b2p[TMP_REG_1], b2p[TMP_REG_1], b2p_bpf_array));
			      ...
			      u32 *addrs, bool extra_pass)
{
	...
	for (i = 0; i < flen; i++) {
		...
		case BPF_ALU64 | BPF_ADD | BPF_X: /* dst += src */
			EMIT(PPC_ADD(dst_reg, dst_reg, src_reg));
		...
		case BPF_ALU64 | BPF_SUB | BPF_K: /* dst -= imm */
			...
			if (imm) {
				if (imm >= -32768 && imm < 32768)
					...
					EMIT(PPC_ADD(dst_reg, dst_reg, b2p[TMP_REG_1]));
				}
			}
			goto bpf_alu32_trunc;
...
emit_clear:
		...
		case BPF_STX | BPF_XADD | BPF_W:
			...
			EMIT(PPC_ADD(b2p[TMP_REG_2], b2p[TMP_REG_2], src_reg));
			/* store result back */
		...
		case BPF_LDX | BPF_MEM | BPF_W:
			EMIT(PPC_LWZ(dst_reg, src_reg, off));
			if (insn_is_zext(&insn[i + 1]))
				addrs[++i] = ctx->idx * 4;
			break;
		/* dst = *(u64 *)(ul) (src + off) */
